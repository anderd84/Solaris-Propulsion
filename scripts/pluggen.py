...
figRao = plots.CreateNonDimPlot()
plots.PlotContour(figRao, cont, Rt, Tt, Re)
plots.PlotField(figRao, field, Re.m)
plt.plot([cont[-1].x], [cont[-1].r], 'ro')
plt.plot([cont[0].x], [cont[0].r], 'bo')
...
# p = Q_(14.7, unitReg.psi)
p = units.PRESCOTT_PRESSURE
ic(p.to(unitReg.psi))
rlines, llines, streams = analysis.CalculateComplexField(cont, p, exhaust, 1, Tt, Rt, Re.magnitude, 50, 0, 3)
istream = streams[0]
